...
\emph{entities}\index{entity} (see \S\ref{sec:genericity}). The Pajé
...
\item the \%EventDef line contains the name and the unique number of an
  ...
  \S\ref{sec:containers} and \S\ref{sec:entities}) are thus
...
For the example in section~\ref{s:generic}, one would need two events (see
...
``Name'' or ``Alias'' (if it has one) field.
